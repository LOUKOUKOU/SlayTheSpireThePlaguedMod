...
public class DrawPileToHandAction extends AbstractGameAction {
    ...
    public DrawPileToHandAction(int amount, AbstractCard.CardType type, Boolean byCost, Integer cost) {
        this.p = AbstractDungeon.player;
        this.setValues(this.p, this.p, amount);
        this.actionType = ActionType.CARD_MANIPULATION;
        this.duration = Settings.ACTION_DUR_MED;
        this.typeToCheck = type;
        this.byCost = byCost != null ? byCost : false ;
    ...
    public void update() {
        if (this.duration == Settings.ACTION_DUR_MED) {
            ...
            for(AbstractCard c : this.p.drawPile.group) {
                if (c.type == this.typeToCheck && (!this.byCost || c.cost == this.cost)) {
